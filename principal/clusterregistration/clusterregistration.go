...
func NewClusterRegistrationManager(enabled bool, namespace, resourceProxyAddress, caCertPath string,
...
func (mgr *ClusterRegistrationManager) RegisterCluster(ctx context.Context, agentName string) error {
	...
	if exists {
		...
		if err != nil {
			logCtx.WithError(err).Warn("Could not validate cluster token, will refresh")
			valid = false
		}
	...
	logCtx.Info("Creating cluster secret for agent")

	if err := cluster.CreateClusterWithBearerToken(ctx, mgr.kubeclient, mgr.namespace, agentName, mgr.resourceProxyAddress, mgr.issuer, mgr.caCertPath); err != nil {
		return fmt.Errorf("failed to create cluster secret: %v", err)
	}

	logCtx.Info("Agent cluster registration completed successfully")
