...
func Test_UpdateClusterInfo(t *testing.T) {
	// Force cache expiration to ensure clean state for the test
	...
	t.Run("Update cluster info when agent is connected", func(t *testing.T) {
		...
		clusterInfo, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, m.mapping(agentName).Server, m.redisAddress, m.redisCompressionType, cache.SourcePrincipal)
		require.NoError(t, err)
		...
		clusterInfoNew, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, m.mapping(agentName).Server, m.redisAddress, m.redisCompressionType, cache.SourcePrincipal)
		require.NoError(t, err)
...
func Test_SetClusterCacheStats(t *testing.T) {
	...
	t.Run("Update cluster cache stats preserves connection status", func(t *testing.T) {
		...
		initialInfo, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, m.redisCompressionType, cache.SourcePrincipal)
		require.NoError(t, err)
		...
		updatedInfo, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, m.redisCompressionType, cache.SourcePrincipal)
		require.NoError(t, err)
...
func Test_GetClusterInfo(t *testing.T) {
	...
	t.Run("Get cluster info successfully", func(t *testing.T) {
		...
		retrievedInfo, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, m.redisCompressionType, cache.SourcePrincipal)
		require.NoError(t, err)
	...
	t.Run("Get cluster cache info for non-existent cluster", func(t *testing.T) {
		...
		_, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, "https://non-existent-cluster", m.redisAddress, m.redisCompressionType, cache.SourcePrincipal)
		require.Error(t, err)
	})
}
...
func Test_SetAgentConnectionStatus(t *testing.T) {
	...
	t.Run("SetAgentConnectionStatus resets cache information on failure", func(t *testing.T) {
		...
		cluster := m.mapping(agentName)
		require.NotNil(t, cluster)
		preInfo, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, m.redisCompressionType, cache.SourcePrincipal)
		require.NoError(t, err)
		...
		postInfo, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, m.redisCompressionType, cache.SourcePrincipal)
		require.NoError(t, err)
