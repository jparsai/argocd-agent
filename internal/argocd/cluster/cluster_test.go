...
func Test_SetAgentConnectionStatus(t *testing.T) {
	...
	t.Run("SetAgentConnectionStatus preserves cache information", func(t *testing.T) {
		// First set some cache stats
		m.SetClusterCacheStats(appv1.ClusterInfo{
		...
		// Verify cache stats are set
		cluster := m.mapping(agentName)
		require.NotNil(t, cluster)

		initialInfo, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, m.redisCompressionType)
		require.NoError(t, err)
		require.Equal(t, int64(10), initialInfo.ApplicationsCount)
		require.Equal(t, int64(25), initialInfo.CacheInfo.APIsCount)
		require.Equal(t, int64(300), initialInfo.CacheInfo.ResourcesCount)

		// Now update connection status
		connectionTime := time.Now()
		m.SetAgentConnectionStatus(agentName, appv1.ConnectionStatusSuccessful, connectionTime)

		// Verify that connection status is updated but cache information is preserved
		updatedInfo, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, m.redisCompressionType)
		require.NoError(t, err)
		require.Equal(t, appv1.ConnectionStatusSuccessful, updatedInfo.ConnectionState.Status)
		require.Equal(t, int64(10), updatedInfo.ApplicationsCount)
		require.Equal(t, int64(25), updatedInfo.CacheInfo.APIsCount)
		require.Equal(t, int64(300), updatedInfo.CacheInfo.ResourcesCount)
	})
}
