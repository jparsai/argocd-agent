...
func (m *Manager) SetAgentConnectionStatus(agentName, status string, t time.Time) {
	...
	if err := m.setClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, agentName, cluster.Name, m.redisCompressionType,
		&appv1.ClusterInfo{
...
func (m *Manager) refreshClusterInfo() {
	...
	for agentName, cluster := range m.clusters {
		...
		// Re-save same info.
		if err := m.setClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress,
...
func (m *Manager) SetClusterCacheStats(clusterInfo appv1.ClusterInfo, agentName string) {
	...
	if err := m.setClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, agentName, cluster.Name, m.redisCompressionType, newClusterInfo); err != nil {
...
func (m *Manager) setClusterInfo(ctx context.Context, kubeclient kubernetes.Interface, namespace, clusterServer, redisAddress, agentName, clusterName string,
