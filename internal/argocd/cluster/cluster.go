...
func (m *Manager) SetAgentConnectionStatus(agentName, status string, t time.Time) {
	...
	// Get existing cluster info to preserve cache information
	...
	// Create new cluster info with provided connection state and time
	...
	// Preserve existing cache information if available
	if existingClusterInfo != nil {
		newClusterInfo.ApplicationsCount = existingClusterInfo.ApplicationsCount
		newClusterInfo.CacheInfo.APIsCount = existingClusterInfo.CacheInfo.APIsCount
		newClusterInfo.CacheInfo.ResourcesCount = existingClusterInfo.CacheInfo.ResourcesCount
	...
	if err := setClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server,
		m.redisAddress, agentName, cluster.Name, m.redisCompressionType, newClusterInfo); err != nil {
...
// SetClusterCacheStats updates cluster cache info with Application, Resource and API stats at principal.
...
func (m *Manager) SetClusterCacheStats(clusterInfo appv1.ClusterInfo, agentName string) {
	...
	// Get existing cluster info to preserve connection status
	existingClusterInfo, err := GetClusterInfo(m.ctx, m.kubeclient, m.namespace, cluster.Server, m.redisAddress, m.redisCompressionType)
	if err != nil && !errors.Is(err, cacheutil.ErrCacheMiss) {
		log().Errorf("failed to get existing cluster info for cluster: '%s' mapped with agent: '%s'. Error: %v", cluster.Name, agentName, err)
		// Continue with default values if we can't get existing info
	...
	// Preserve existing connection status if available
	if existingClusterInfo != nil {
...
// GetClusterInfo retrieves the cluster ClusterInfo for a given cluster server from the cache.
...
// setClusterInfo saves the given cluster info in the cache.
