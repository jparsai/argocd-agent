...
type Agent struct {
	...
	// inflightExec blocks starting a duplicate web terminal session for the same request UUID.
	inflightExec map[string]struct{}
...
func NewAgent(ctx context.Context, client *kube.KubernetesClient, namespace string, opts ...AgentOption) (*Agent, error) {
	a := &Agent{
		...
		inflightExec: make(map[string]struct{}),
	}
