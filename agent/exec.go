...
// processIncomingExecRequest handles incoming web terminal exec requests from the principal.
...
func (a *Agent) processIncomingExecRequest(ev *event.Event) error {
	...
	a.inflightMu.Lock()
	if a.inflightExec == nil {
		a.inflightExec = make(map[string]struct{})
	}
	if _, dup := a.inflightExec[execReq.UUID]; dup {
		a.inflightMu.Unlock()
		log().WithField("session_uuid", execReq.UUID).Warn("duplicate exec request; already processing")
		return nil
	}
	a.inflightExec[execReq.UUID] = struct{}{}
	a.inflightMu.Unlock()
	defer func() {
		a.inflightMu.Lock()
		delete(a.inflightExec, execReq.UUID)
		a.inflightMu.Unlock()
	}()
	...
	stream, err := execStreamClient.StreamExec(ctx)
	if err != nil {
	...
	if err := stream.Send(&execstreamapi.ExecStreamData{RequestUuid: execReq.UUID}); err != nil {
		logCtx.WithError(err).Error("Failed to send initial exec frame")
		return err
	}

	// Execute the K8s exec API call
	if err := a.execInPod(ctx, stream, execReq, logCtx); err != nil {
...
func (a *Agent) execInPod(ctx context.Context, stream execstreamapi.ExecStreamService_StreamExecClient, execReq *event.ContainerExecRequest, logCtx *logrus.Entry) error {
	...
	// Send initial terminal size to the shell
	...
	// Close the stream to signal the end of the exec session
	streamHandler.close()
	...
	// Send EOF to principal to signal the end of the exec session
	_ = stream.Send(&execstreamapi.ExecStreamData{
		RequestUuid: execReq.UUID,
		Eof:         true,
	})
...
// close safely closes the done channel to signal the end of the exec session
func (h *execStreamHandler) close() {
	h.closeOnce.Do(func() {
		close(h.done)
	})
}
...
func (h *execStreamHandler) receiveFromPrincipal() {
	defer func() {
		// Close done channel to signal the end of the exec session
		h.close()
		close(h.stdin)
	}()
	...
	for {
		...
		if msg.Eof {
			...
			// Close stdin channel to signal the end of the exec session
			return
		}
