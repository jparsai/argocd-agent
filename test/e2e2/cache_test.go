...
const (
	...
	AgentName_Managed = "agent-managed"
)
...
func (suite *CacheTestSuite) SetupTest() {
	...
	if !fixture.IsProcessRunning(AgentName_Managed) {
		// Start the agent if it is not running and wait for it to be ready
		suite.Require().NoError(fixture.StartProcess(AgentName_Managed))
		fixture.CheckReadiness(suite.T(), AgentName_Managed)
	} else {
...
func (suite *CacheTestSuite) Test_CacheRecreatedOnRestart() {
	...
	fixture.RestartAgent(suite.T(), AgentName_Managed)
	fixture.CheckReadiness(suite.T(), AgentName_Managed)
...
func (suite *CacheTestSuite) Test_RevertManagedClusterOfflineChanges() {
	...
	requires.Eventually(func() bool {
		return fixture.IsProcessRunning(AgentName_Managed)
	}, 60*time.Second, 1*time.Second)

	requires.NoError(fixture.StopProcess(AgentName_Managed))
	requires.Eventually(func() bool {
		return !fixture.IsProcessRunning(AgentName_Managed)
	}, 60*time.Second, 1*time.Second)
	...
	requires.NoError(fixture.StartProcess(AgentName_Managed))
	fixture.CheckReadiness(suite.T(), AgentName_Managed)
	validateAppReverted(suite.Ctx, suite.ManagedAgentClient, &app, key, requires, suite.T())
}
