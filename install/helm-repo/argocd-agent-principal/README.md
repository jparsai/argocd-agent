...
| principal.spire | object | `{"enabled":false,"hostSocketDir":"/run/spire/agent-sockets","mountMethod":"hostPath","socketPath":"unix:///run/spire/agent-sockets/spire-agent.sock"}` | SPIRE integration for automatic certificate and identity provisioning. |
| principal.spire.enabled | bool | `false` | Enable SPIRE-based TLS. When enabled, TLS credentials are obtained from the SPIRE Workload API instead of static Kubernetes secrets, and agents authenticate using JWT-SVIDs. |
