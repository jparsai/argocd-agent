...
- Validates agent JWT-SVIDs using the SPIRE trust bundle
- Defaults `--auth` to `spiffe-jwt:spiffe://[^/]+/(.+)` if not explicitly set
...
- Uses SPIRE SVIDs for TLS (instead of `argocd-agent-client-tls`)
- Defaults `--creds` to `spiffe-jwt:` if not explicitly set
...
With the default auth regex `spiffe-jwt:spiffe://[^/]+/(.+)`, the agent name extracted from `spiffe://example.org/argocd/agent/cluster-01` would be `argocd/agent/cluster-01`.
